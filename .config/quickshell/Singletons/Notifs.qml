...
Singleton {
    ...
    /**
     * Focus a Niri window matching the notification's app by matching app_id
     * or title against desktopEntry/appName.
     */
    function raiseWindow(n) {
        ...
        Quickshell.execDetached(["sh", "-c",
            "wid=$(niri msg -j windows | jq -r --arg q \"$1\" 'first(.[] | select((.app_id | ascii_downcase | contains($q)) or (.title | ascii_downcase | contains($q))) | .id)'); [ -n \"$wid\" ] && niri msg action focus-window --id \"$wid\"",
            "sh", token]);
    }
